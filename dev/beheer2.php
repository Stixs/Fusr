...
$parameters = array(':specialiteit'=>$specialiteit,
	...
	$sth = $pdo->prepare('INSERT INTO branches (specialiteit, branche_id)VALUES(:specialiteit, :branche)');
...
$parameters = array(':specialiteit'=>$specialiteit[$keuze],
					':keuze'=>$keuze);
$sth = $pdo->prepare('UPDATE branches SET specialiteit=:specialiteit WHERE specialiteit_id = :keuze');
$sth->execute($parameters);
...
$sth = $pdo->prepare('DELETE FROM branches WHERE specialiteit_id = :keuze');
$sth->execute($parameters);
}
...
{
	$branche = $_SESSION['branche'];
	$sth = $pdo->prepare("SELECT * FROM branches WHERE branche_id = " . $_SESSION['branche'] . " AND specialiteit LIKE '%" . $beperk. "%'");
	$sth->execute();
...
?>
	...
	<div class="col-xs-12 ContentPadding" style="padding-top:20px;">
		...
		<form class="form-inline" action="" method="post">
			...
			$sth = $pdo->prepare("select * from branches WHERE branche_id = " . $_SESSION['branche'] . " ORDER BY specialiteit");
			$sth->execute();
			$first = true;
			while($row = $sth->fetch())
			{
