...
function speciBeheer($pdo, $id = '') {
	$html = '<label for="sel' . $id . '">Specialiteit:</label>';
	$html .= '<select class="form-control" name="specialiteiten[]">';
			...
			while($row = $sth->fetch())
				{
					...
					if($row['naam'] == $keuze)
					{
						$html .= '<option value="'.$row['naam'].'" selected="selected">'.$row['naam'].'</option>';
					}
					else
					{
						$html .= '<option value="'.$row['naam'].'">'.$row['naam'].'</option>';
					}
				}
	$html .= '</select>';
    return $html;
